/************************** AOL CONFIG SECTION **************************************/
/* Site: MapQuest Enterprise w/ chg for s_vp_getCGI(cid), sp1, sp2, s_linkInternalFilters and s_account   Date: 6/27/05     IE: Michelle Gildea   Version: 1 */

var s_account,s_pfxID,s_prop1,s_prop2,s_prop6,s_prop7,s_campaign,s_prop16,s_eVar16,s_eVar17
var s_currencyCode="USD"
var s_account="aolwpmq,aolsvc"  /* for production var s_account= "devaolwpmq"  */
/* Link Tracking Config */
var s_trackDownloadLinks=true
if (typeof s_trackExternalLinks=='undefined'){var s_trackExternalLinks=true;}
var s_trackInlineStats=true
var s_linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,qt,nsv,ra,ram,rm"
var s_linkInternalFilters="javascript:,mapquest.com"
if (typeof s_linkLeaveQueryString=='undefined'){var s_linkLeaveQueryString=false;}
if ((!s_linkTrackVars)||s_blank(s_linkTrackVars)){var s_linkTrackVars="None";}
if ((!s_linkTrackEvents)||s_blank(s_linkTrackEvents)){var s_linkTrackEvents="None";}
s_prop1=s_prop1;
s_prop2=s_prop2;
/* Plugin Config */
...
function s_doPlugins() {
	...
	/* WAGS Code for auth and user state */
	if (s_c_r("SNS_AA")=="2" || s_c_r("SNS_AA")=="1"){s_prop13="authenticated";}
	else{s_prop13="non-authenticated";}
 
	var ck=s_c_r ("RSP_COOKIE");if (ck!="" && ck!=null){ck=s_c_r ("RSP_COOKIE").split("&");	for(i=0;i<ck.length;i++){if(ck[i].split("=")[0]=="type"){if(ck[i].split("=")[1]=="0"){var cv="member";}else{var cv="registered"}}}}
	else { var cv="visitor";}s_prop11=cv;

	/* END WAGS */
	
	/*Campaign and Network Tracking*/
	if ((!s_campaign)||s_blank(s_campaign)){s_vp_getCGI('s_campaign','ncid');}	
	if ((!s_eVar16)||s_blank(s_eVar16)){s_vp_getCGI('s_eVar16','icid');}
	if ((!s_eVar17)||s_blank(s_eVar17)){s_vp_getCGI('s_eVar17','ecid');}	
	if ((!s_prop16)||s_blank(s_prop16)){s_vp_getCGI('s_prop16','cid');}	
	
	/*
	 * SEM/SEO code.  If referrer is NOT blank or does NOT contain 'aol.' OR "instore." then
	 * overwrite cookie value and store in s_prop14.  
	 * Else read cookie and store in s_prop14.
	 * VISTA rule will look at referrer and determine whether or not it matches search engine
	 * string.  If yes, it will put data in separate report suite.
	 * WE WILL NEED TO ADD OTHER STRINGS SO WE DON'T OVERWRITE REFERRERS (E.G. mapquest)
	 * ALSO NEED TO DETERMINE EXPIRATION DATE.  Currently set to 30 days
	 */
	var rt=(s_gg('referrer'))?s_gg('referrer'):s_gtfs().document.referrer;
	rt=rt.toLowerCase();
	if (rt){ if(!(rt.indexOf("google.")>-1||rt.indexOf("webcrawler.")>-1||rt.indexOf("yahoo.")>-1
	||rt.indexOf("dogpile.")>-1||rt.indexOf("lycos.")>-1||rt.indexOf("altavista.")>-1
	||rt.indexOf("looksmart.")>-1||rt.indexOf("msn.")>-1||rt.indexOf("excite.")>-1
	||rt.indexOf("clickserve.")>-1||rt.indexOf("search.aol.")>-1))rt="" }
	s_vp_setAndPersistValue('s_prop14',rt,30)
}

/************************** PLUGINS SECTION *************************/
/* You may insert any plugins you wish to use here.                 */
/*
 * Plugin: Store Value in Cookie (destination, source, expire)
 */
...
/*
 * Plugin: Check for "unavailable"
 */
function s_uv(skd){
	if (skd.toLowerCase()=="unavailable"){return true;}
	else {return false;}
}
/*
 * Plugin: Check for blank value
 */
function s_blank(sp){
	if(sp==null){return true;}
	for(var i=0;i<sp.length;i++){
	if((sp.charAt(i)!='')&&(sp.charAt(i)!="\t")&&(sp.charAt(i)!="\n")&&(sp.charAt(i)!="\r")){return false;}
	}
	return true;}
/*
